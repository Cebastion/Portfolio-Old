  ...
  const CreateWork = () => {
    try {
      if(Photo) {
        ...
        axios.post(`http://localhost:5500/edit_work/?_id=${Work._id}`, formData).then(res => {
          SetActivePopEdit(false)
          SetPreview('')
        })
      } else {
        ...
        <main className={style.block__form}>
          <div className={style.form__img}>
            ...
            <label>
              ...
              <div className={style.add_img_block} style={{ backgroundImage: `url(${Preview ? Preview : `http://localhost:5500/image/${Work._id}`})` }}>
              </div>
            </label>
          </div>
